...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($userData) {
        ...
        if ($user->updateResetToken($email, $resetToken)) {

            $resetLink = "localhost/projet_WEB_PHP_YUG4/Controller/reset_password_confirm.php?email=$email&token=" . urlencode($resetToken);
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'devsarealsohumansV2@outlook.fr'; 
                $mail->Password = 'Doranco!123456789'; 
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;

                $mail->setFrom('devsarealsohumansV2@outlook.fr', 'Sacha Thibault');
                $mail->addAddress($email);
                $mail->isHTML(true);
                $mail->Subject = 'Réinitialisation de mot de passe';
                $mail->Body = "Cliquez sur le lien suivant pour réinitialiser votre mot de passe : $resetLink";

                $mail->send();
                $_SESSION['reset_status'] = "Un e-mail a été envoyé avec le lien de réinitialisation.";
            } catch (Exception $e) {
                $_SESSION['reset_status'] = "Erreur lors de l'envoi de l'e-mail : " . $mail->ErrorInfo;
            }
